...
ESPI2CScanner::ESPI2CScanner(){};

void ESPI2CScanner::begin(TwoWire *_WirePort) { WirePort = _WirePort; }

#ifdef DEBUG
void ESPI2CScanner::scanAll() {
...
boolean ESPI2CScanner::scan(byte address) {
  byte status;
  WirePort->beginTransmission(address);
  status = WirePort->endTransmission();
  if (status == 0) {
#ifdef DEBUG
    Serial << endl << F(" - Item Found [0x");
    if (address < 16) {
      Serial << F("0");
    }
    Serial << _HEX(address) << F("] : ") << getName(address) << endl;
#endif
    return true;
  } else {
#ifdef DEBUG
...
const char *ESPI2CScanner::getName(byte deviceAddress) {
